...
+ Aufspaltung der Summe in zwei Teilsummen
  $$||a||_p :=
  (\sum_{i=0}^{N-1}\lfloor|a_i|\rfloor^p)^{1/p}=(\sum_{i=1}^{m-1}\lfloor|a_i|\rfloor^p +
...
val v2 = t2.join  
  ...
  + =t= ist eine =task= die, die den Wert von =e= berechnet.
...
+ /Testen/ soll zeigen, dass für konkrete Anwednungsfälle das Programm
...
+ den wahlfreien Zugriff auf die Elemente (Listen sind hier
  ungeeignet)
*** Sequentialle Version von =map= für Bäume
#+BEGIN_SRC scala
